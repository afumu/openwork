...
export function fetchChatAPIProcess<T = any>(params: {
  ...
  return new Promise((resolve, reject) => {
    ...
    fetchStream('/openwork/chat-process', fetchOptions, chunk => {
...
export function fetchChatStopAPIProcess<T = any>(params: { groupId?: number; chatId?: number }) {
  return post<T>({
    url: '/openwork/chat-stop',
    data: {
...
export function fetchPptCoverAPIProcess<T>(data: {
  ...
  return post<T>({ url: '/openwork/ppt-cover', data }) as Promise<T>
}

/* TTS 文字转语音 */
export function fetchTtsAPIProcess<T>(data: { chatId: number; prompt: string }): Promise<T> {
  return post<T>({ url: '/openwork/tts-process', data }) as Promise<T>
}
