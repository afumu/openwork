...
interface Emit {
  ...
  (ev: 'open-group-chat', topic: string): void
  (ev: 'resume-group-chat'): void
}

interface Props {
  dataSourcesLength: number
  hasGroupChatResume?: boolean
  showGroupChatEntry?: boolean
}
...
const researchMode = computed({
  get: () => chatStore.researchMode,
  set: value => {
    chatStore.setResearchMode(value)
  },
})
...
const groupChatDisabled = computed(() => !prompt.value || prompt.value.trim() === '')
...
const handleOpenGroupChat = () => {
  if (groupChatDisabled.value) return
  emit('open-group-chat', prompt.value?.trim() || '')
}

const handleResumeGroupChat = () => {
  emit('resume-group-chat')
}
...
const placeholderText = computed(() => {
  ...
  if (usingDeepThinking.value) {
    activeFeatures.push('用 AI 推理拆解复杂问题、影响和关键争议')
  }

  if (usingNetwork.value) {
    activeFeatures.push('检索最新资料、公开来源和背景信息')
  }
  ...
  return `输入要研究的任务、项目、公司、人物或议题`
})

const shouldShowNetworkSearch = computed(() => {
  // 网络搜索是当前对话的外部工具开关，不再跟随单个模型的 isNetworkSearch 能力隐藏。
  return true
})
          ...
          <div ref="buttonContainerRef" class="flex justify-between flex-grow w-full pb-2">
            ...
            <div class="flex justify-start items-center">
              ...
              <div class="group relative">
                <div
                  class="btn-pill btn-md mx-1"
                  :class="[researchMode ? 'btn-pill-active' : '']"
                ...
                >
                  <TreeDiagram size="15" />
                  <span v-if="shouldShowButtonText" class="ml-1">研究</span>
                </div>
                <div v-if="!isMobile" class="tooltip tooltip-top">
                  使用更深的搜索策略，帮助完成资料收集与验证
                </div>
              </div>
            ...
            <div class="flex justify-end items-center mr-1">
              <GroupChatEntry
                v-if="showGroupChatEntry"
                :disabled="groupChatDisabled"
                :hasResume="hasGroupChatResume"
                @open="handleOpenGroupChat"
                @resume="handleResumeGroupChat"
              />
