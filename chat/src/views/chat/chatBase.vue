...
// ============== 组件导入 ==============
import { fetchChatAPIProcess, fetchChatStopAPIProcess } from '@/api'
import { fetchArtifactListAPI } from '@/api/artifacts'
...
import { Close, DocDetail, DropDownList } from '@icon-park/vue-next'
...
import ArtifactsDrawer from './components/ArtifactsDrawer.vue'
import AppTips from './components/AppTips/index.vue'
import DiscussionRoom from './components/group-chat/DiscussionRoom.vue'
import FooterComponent from './components/Footer/index.vue'
import GroupChatBuilderDrawer from './components/group-chat/GroupChatBuilderDrawer.vue'
...
import { flushStreamJsonBuffer, parseStreamJsonLines } from '@/utils/streamJsonLines'
...
interface ToolExecutionStreamItem {
  tool_call_id: string
  tool_name: string
...
const groupDiscussionFeatureEnabled = false
...
const handleResumeGroupChat = () => {
  if (!groupChat.lastRoom.value) {
    ms.info('还没有可恢复的专家讨论房间')
    return
  }
...
const handleDiscussionSend = ({
  ...
  groupChat.sendUserMessage(content, isInterrupt)
  groupChat.scheduleAdvance(roomId, content)
}

const handleContinueDiscussion = () => {
  const roomId = groupChat.activeRoom.value?.id
  groupChat.scheduleAdvance(roomId)
}

const handleAddDiscussionExpert = (name: string) => {
  const added = groupChat.addExpertToRoom(name)
  if (!added) {
    ms.info('该专家已经在房间中了')
    return
  }
  ms.success('新专家已加入，将在下一轮参与')
}

const handleStopDiscussion = () => {
  const stopped = groupChat.stopDiscussion()
  if (!stopped) return
  ms.info('本场讨论已停止')
}

const handleUpdateDiscussionParticipantPersona = ({
  participantId,
  personaPrompt,
}: {
  ...
  groupChat.updateParticipantPersona(participantId, personaPrompt)
  ms.success('已更新当前专家的人设 prompt')
}

const handleRegenerateDiscussionParticipantPersona = ({
  participantId,
  extraNeed,
}: {
  participantId: string
  ...
  groupChat.regenerateParticipantPersona(participantId, extraNeed)
  ms.success('已根据你的要求重新生成专家人设')
}
const artifactsInitialPath = ref('')
const conversationArtifactCount = ref(0)
const conversationArtifactLoading = ref(false)
let conversationArtifactTimer: number | null = null
...
watch(
  () => [activeGroupId.value, dataSources.value.length, chatStore.isStreamIn] as const,
  () => {
    restartConversationArtifactPolling()
  },
  { immediate: true }
)
...
onBeforeUnmount(() => {
  clearConversationArtifactPolling()
})
...
const onConversation = async ({
  ...
  addGroupChat({
    ...
    imageUrl: imageUrl || '',
    researchMode: chatStore.researchMode,
  })
  ...
  const handleStreamResponseModel = async () => {
    ...
    const buildAssistantPatch = (): Chat.Chat => ({
      chatId: Number(assistantLogId),
      content: displayedText,
      reasoningText: displayedReasoningText,
      tool_execution: toolExecution,
      stream_segments: serializeStreamSegments(),
    ...
    const handleStreamJsonObject = (jsonObj: Record<string, any>) => {
      ...
      if (jsonObj.content) {
        ...
        const newText = jsonObj.content[0].text.replace(/\\n/g, '\n').replace(/\\t/g, '\t')
        ...
        if (isCacheEnabled) {
          ...
          appendTextStreamSegment(newText)
      ...
      if (jsonObj.fileVectorResult) fileVectorResult = jsonObj.fileVectorResult
      if (jsonObj.artifact_files) {
        artifactFiles = Array.isArray(jsonObj.artifact_files)
          ? JSON.stringify(jsonObj.artifact_files)
          : String(jsonObj.artifact_files)
      }
    ...
    const appendTextStreamSegment = (textChunk: string) => {
      ...
      streamSegments = [
        ...streamSegments,
        {
          id: `text-${streamSegments.length + 1}-${Date.now()}`,
          type: 'text',
          text: textChunk,
        },
      ]
    ...
    const flushPendingTextBuffer = () => {
      ...
      appendTextStreamSegment(pendingText)
    ...
    const upsertToolExecutionItems = (items: ToolExecutionStreamItem[]) => {
      if (!items.length) return false

      let hasChanges = false

      items.forEach((item: ToolExecutionStreamItem, index: number) => {
        if (!item || typeof item !== 'object') return

        const toolCallId =
          typeof item.tool_call_id === 'string' && item.tool_call_id.trim()
            ? item.tool_call_id
            : `${item.tool_name || 'tool'}-${index + 1}`

        lastVisibleSegmentType = 'tool_execution'

        const existingIndex = toolSegmentIndexById.get(toolCallId)
        if (existingIndex !== undefined) {
          const existingSegment = streamSegments[existingIndex]
          if (existingSegment?.type === 'tool_execution') {
            streamSegments[existingIndex] = {
              ...existingSegment,
              ...item,
        ...
        toolSegmentIndexById.set(toolCallId, streamSegments.length)
        streamSegments = [
          ...streamSegments,
          {
            id: `tool-${toolCallId}`,
    ...
    const upsertToolExecutionSegments = (rawToolExecution: string) => {
      if (!rawToolExecution) return false

      try {
        const parsed = JSON.parse(rawToolExecution)
        if (!Array.isArray(parsed)) return false

        return upsertToolExecutionItems(parsed as ToolExecutionStreamItem[])
      } catch (error) {
        return false
      }
    }
    ...
    const updateDisplay = () => {
      ...
      if (hasTextToDisplay) {
        ...
        appendTextStreamSegment(nextTextChunk)
    ...
    try {
      await fetchChatAPIProcess({
        ...
        onDownloadProgress: ({ event }) => {
          const responseText = String(event.target.responseText || '')
          ...
          try {
            const parseResult = parseStreamJsonLines<Record<string, any>>(
              responseText,
              streamJsonBuffer
            )
            streamJsonBuffer = parseResult.buffer
            parseResult.items.forEach(handleStreamJsonObject)
          } catch (error) {
            // 整体解析错误，忽略
          }
        },
      ...
      flushStreamJsonBuffer<Record<string, any>>(streamJsonBuffer).forEach(handleStreamJsonObject)
      streamJsonBuffer = ''
    } catch (error) {
...
const pauseRequest = async () => {
  const currentStreamRequest = activeStreamRequest.value
  activeStreamRequest.value = null
  controller.value.abort()
  chatStore.setStreamIn(false)
  if (currentStreamRequest?.groupId || currentStreamRequest?.chatId) {
    try {
      await fetchChatStopAPIProcess({
        groupId: currentStreamRequest.groupId,
        chatId: currentStreamRequest.chatId,
      })
    } catch (error) {
...
function toggleArtifactsDrawer(visible?: boolean) {
  if (typeof visible === 'boolean') {
    artifactsDrawerVisible.value = visible
    if (!visible) {
      artifactsInitialPath.value = ''
    }
    return
  }

  artifactsDrawerVisible.value = !artifactsDrawerVisible.value
  if (!artifactsDrawerVisible.value) {
    artifactsInitialPath.value = ''
  }
}
...
function unwrapArtifactPayload<T>(payload: any): T | null {
  ...
  if ('workspaceFiles' in payload || 'runs' in payload) return payload as T
  if ('data' in payload) return unwrapArtifactPayload<T>(payload.data)
  return null
}

function countConversationArtifacts(manifest: ArtifactManifest | null) {
  if (!manifest) return 0
  if (manifest.workspaceFiles?.length) return manifest.workspaceFiles.length
  return (manifest.runs || []).reduce((total, run) => total + (run.files?.length || 0), 0)
}

async function loadConversationArtifactCount(showLoading = false) {
  const groupId = Number(activeGroupId.value || 0)
  if (!groupId || !dataSources.value.length) {
    conversationArtifactCount.value = 0
    return
  }

  if (showLoading) conversationArtifactLoading.value = true
  try {
    const res: any = await fetchArtifactListAPI({ groupId })
    const manifest = unwrapArtifactPayload<ArtifactManifest>(res)
    conversationArtifactCount.value = countConversationArtifacts(manifest)
  } catch (_error) {
    conversationArtifactCount.value = 0
  } finally {
    conversationArtifactLoading.value = false
  }
}

function clearConversationArtifactPolling() {
  if (!conversationArtifactTimer) return
  window.clearInterval(conversationArtifactTimer)
  conversationArtifactTimer = null
}

function restartConversationArtifactPolling() {
  clearConversationArtifactPolling()
  if (!activeGroupId.value || !dataSources.value.length) return

  void loadConversationArtifactCount(true)
  if (!chatStore.isStreamIn) return

  conversationArtifactTimer = window.setInterval(() => {
    void loadConversationArtifactCount(false)
  }, 4000)
}
...
provide('toggleArtifactsDrawer', toggleArtifactsDrawer)
provide('openArtifactPreview', openArtifactPreview)
provide('artifactsDrawerVisible', artifactsDrawerVisible)
      ...
      <template v-else>
              ...
              <template v-if="dataSources.length">
                ...
                >
                  <Message
                    ...
                    :stream_segments="item.stream_segments"
        ...
        >
          <FooterComponent
            ...
            @open-group-chat="handleOpenGroupChat"
            @resume-group-chat="handleResumeGroupChat"
            :dataSourcesLength="dataSources.length"
            :hasGroupChatResume="groupDiscussionFeatureEnabled && !!groupChat.lastRoom.value"
            :showGroupChatEntry="groupDiscussionFeatureEnabled"
          >
            ...
            <template #before-footer>
              ...
              >
                ...
                >
                  ...
                  <span v-if="conversationArtifactCount" class="conversation-artifacts-count">
                    {{ conversationArtifactCount }}
                  </span>
                  <span
                    v-else-if="conversationArtifactLoading"
                    class="conversation-artifacts-loading"
                  >
                    ...
                  </span>
                </button>
              ...
              <div
                v-if="
                  groupDiscussionFeatureEnabled &&
                  groupChat.lastRoom.value &&
                  !groupChat.roomVisible.value
                "
                class="mx-auto mb-3 max-w-3xl px-4"
              >
                <button
                  type="button"
                  class="w-full rounded-2xl border border-primary-200 bg-primary-50/80 px-4 py-3 text-left dark:border-primary-500/20 dark:bg-primary-500/10"
                  @click="handleResumeGroupChat"
                >
                  <div class="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <div class="mt-1 text-xs text-gray-500 dark:text-slate-400">
                        {{ groupChat.lastRoom.value.topic }} · 第
                        {{ groupChat.lastRoom.value.currentRound }} /
                        {{ groupChat.lastRoom.value.maxRounds }} 轮
                      </div>
                    </div>
                    <span class="text-sm text-primary-600 dark:text-primary-300">进入房间</span>
                  </div>
                </button>
              </div>
            </template>
        ...
        <GroupChatBuilderDrawer
          v-if="groupDiscussionFeatureEnabled"
          :visible="groupChat.builderVisible.value"
          :draft="groupChat.activeDraft.value"
          :isDiscovering="groupChat.isDiscovering.value"
          :lastRoom="groupChat.lastRoom.value"
          @close="groupChat.closeBuilder"
          @discover="groupChat.startDiscoverExperts"
          @update:topic="value => groupChat.updateDraftField('topic', value)"
          @update:topicContext="value => groupChat.updateDraftField('topicContext', value)"
          @update:goal="value => groupChat.updateDraftField('goal', value)"
          @update:responseLength="value => groupChat.updateDraftField('responseLength', value)"
          @update:maxRounds="value => groupChat.updateDraftField('maxRounds', value)"
          @toggle-candidate="groupChat.toggleCandidate"
          @update:manualExpertName="value => groupChat.updateDraftField('manualExpertName', value)"
          @add-manual-expert="
            groupChat.addManualExpertToDraft(groupChat.activeDraft.value.manualExpertName)
          "
          @update:candidatePersonaPrompt="
            ({ candidateId, personaPrompt }) =>
              groupChat.updateDraftCandidatePersona(candidateId, personaPrompt)
          "
          @regenerate-candidate-persona="
            ({ candidateId, extraNeed }) =>
              groupChat.regenerateDraftCandidatePersona(candidateId, extraNeed)
          "
          @create-room="handleCreateDiscussionRoom"
          @resume-room="handleResumeGroupChat"
        />

        <DiscussionRoom
          v-if="groupDiscussionFeatureEnabled"
          :visible="groupChat.roomVisible.value"
          :room="groupChat.activeRoom.value"
          @close="groupChat.closeRoom"
          @stop="handleStopDiscussion"
          @send="handleDiscussionSend"
          @continue="handleContinueDiscussion"
          @add-expert="handleAddDiscussionExpert"
          @update:participantPersonaPrompt="handleUpdateDiscussionParticipantPersona"
          @regenerate-participant-persona="handleRegenerateDiscussionParticipantPersona"
        />
...
.conversation-artifacts-entry-wrap {
  display: flex;
...
.conversation-artifacts-entry:hover {
...
.conversation-artifacts-count,
.conversation-artifacts-loading {
  display: inline-flex;
  align-items: center;
...
html.dark .conversation-artifacts-entry,
html[data-theme='dark'] .conversation-artifacts-entry,
.conversation-artifacts-entry-dark {
...
html.dark .conversation-artifacts-entry:hover,
html[data-theme='dark'] .conversation-artifacts-entry:hover,
.conversation-artifacts-entry-dark:hover {
  border-color: rgba(96, 165, 250, 0.28);
  background: rgba(50, 50, 54, 0.92);
}

.conversation-artifacts-entry-dark .conversation-artifacts-count,
.conversation-artifacts-entry-dark .conversation-artifacts-loading,
html.dark .conversation-artifacts-count,
html.dark .conversation-artifacts-loading,
html[data-theme='dark'] .conversation-artifacts-count,
html[data-theme='dark'] .conversation-artifacts-loading {
