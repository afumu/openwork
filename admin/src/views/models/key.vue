...
<script lang="ts" setup>
  ...
  const apiFormatTagTypeMap: Record<string, 'success' | 'warning' | 'info'> = {
    anthropic: 'warning',
    openai: 'success',
  };
  ...
  function handleEditKey(row: any) {
    ...
    nextTick(() => {
      Object.assign(formPackage, {
        ...
        max_tokens,
        proxyUrl,
        apiFormat: apiFormat === 'opensandbox' ? 'anthropic' : apiFormat || 'openai',
        timeout,
        deductType,
        deduct,
        maxRounds,
      ...
      <template #content>
        <div class="text-sm/6">
          ...
          <div>
            ...
            <a href="https://api.openai.com" target="_blank">OpenAI 兼容 API</a>
            格式。
          </div>
