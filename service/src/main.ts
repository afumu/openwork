...
import { RuntimeTerminalGateway } from './modules/aiTool/chat/runtime/runtimeTerminalGateway';
...
async function bootstrap() {
  ...
  app.use(
    ...
    compression({
      filter: (req, res) => {
        // 对流式响应路由禁用压缩
        if (req.path.includes('/api/openwork/chat-process')) {
          return false;
        }
  ...
  if (process.env.ISDEV === 'true') {
    const config = new DocumentBuilder()
      .setTitle('OpenWork API')
      .setDescription('OpenWork 服务 API 文档')
      .setVersion('1.0')
      .addBearerAuth()
      .build();
  ...
  app.get(RuntimeTerminalGateway, { strict: false }).bind(server);
