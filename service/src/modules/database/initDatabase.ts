...
import { PublishAccountEntity } from '../publishAccount/publishAccount.entity';
...
loadEnv({ path: resolveEnvFilePath({ runtimeDir: __dirname }) });
...
async function runAllMigrations() {
  ...
  try {
    ...
    // 3. MEDIUMTEXT类型迁移
    const chatlogColumns = ['content', 'fileVectorResult', 'artifact_files', 'metadata'];
