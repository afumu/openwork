...
export class ChatLogService {
  ...
  async findLatestConversationSummary(groupId: number, userId?: number) {
    const query = this.chatLogEntity
      .createQueryBuilder('log')
      .where('log.groupId = :groupId', { groupId })
      .andWhere('log.isDelete = false')
      .andWhere('log.messageType = :messageType', {
        messageType: CONVERSATION_SUMMARY_MESSAGE_TYPE,
      });
    ...
    return await query.orderBy('log.id', 'DESC').getOne();
  }

  async findConversationContextLogs(groupId: number, afterId = 0) {
    const query = this.chatLogEntity
      .createQueryBuilder('log')
      .where('log.groupId = :groupId', { groupId })
      .andWhere('log.isDelete = false')
      .andWhere('(log.messageType IS NULL OR log.messageType != :summaryType)', {
        summaryType: CONVERSATION_SUMMARY_MESSAGE_TYPE,
      });

    if (afterId > 0) {
      query.andWhere('log.id > :afterId', { afterId });
    }

    const list = await query.orderBy('log.id', 'ASC').getMany();
    return list.map(item => this.toHistoryRecord(item));
  }
  ...
  async saveConversationSummary(params: {
    ...
    return await this.chatLogEntity.save({
      content: params.content,
      groupId: params.groupId,
      userId: params.userId,
      model: params.model || null,
      role: 'system',
      type: ChatType.NORMAL_CHAT,
      status: 3,
      messageType: CONVERSATION_SUMMARY_MESSAGE_TYPE,
      metadata: JSON.stringify({
        coveredChatLogId: params.coveredChatLogId,
  ...
  async exportExcel(body: ExportExcelChatlogDto, res: Response) {
    ...
    const query = this.chatLogEntity
      .createQueryBuilder('log')
      .where('log.type = :type', { type: ChatType.NORMAL_CHAT })
      .andWhere('(log.messageType IS NULL OR log.messageType != :summaryType)', {
        summaryType: CONVERSATION_SUMMARY_MESSAGE_TYPE,
      });

    prompt && query.andWhere('log.prompt LIKE :prompt', { prompt: `%${prompt}%` });
    if (email) {
      const user = await this.userEntity.findOne({ where: { email } });
      user?.id && query.andWhere('log.userId = :userId', { userId: user.id });
    }
  ...
  async querAllChatLog(params: QuerAllChatLogDto, req: Request) {
    ...
    const query = this.chatLogEntity
      .createQueryBuilder('log')
      .where('(log.messageType IS NULL OR log.messageType != :summaryType)', {
        summaryType: CONVERSATION_SUMMARY_MESSAGE_TYPE,
      });
    userId && query.andWhere('log.userId = :userId', { userId });
    prompt && query.andWhere('log.prompt LIKE :prompt', { prompt: `%${prompt}%` });
    type && query.andWhere('log.type = :type', { type });
    model && query.andWhere('log.model = :model', { model });
    const [rows, count] = await query
      .orderBy('log.id', 'DESC')
      .skip((page - 1) * size)
      .take(size)
    ...
    rows.forEach((item: any) => {
      !item.email && (item.email = `${item?.userId}@openwork.local`);
  ...
  async chatHistory(groupId: number, rounds: number) {
    ...
    const list = await this.chatLogEntity
      .createQueryBuilder('log')
      .where('log.groupId = :groupId', { groupId })
      .andWhere('log.isDelete = false')
      .andWhere('(log.messageType IS NULL OR log.messageType != :summaryType)', {
    ...
    const result = list.map(item => this.toHistoryRecord(item)).reverse(); // 添加.reverse()来反转数组，使结果按时间从旧到新排列
  ...
  private toHistoryRecord(item: ChatLogEntity) {
    const {
      id,
      userId,
      role,
  ...
  async querySingleChat(req: Request, params: QuerySingleChatDto) {
    try {
      ...
      const formattedResult = {
        ...
        artifact_files: chatLog.artifact_files || '',
        ...
        tool_execution: chatLog.tool_execution || '',
