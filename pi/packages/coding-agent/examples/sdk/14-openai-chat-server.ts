...
function summarizeAgentEvent(event: AgentEvent | { type: string; [key: string]: unknown }) {
	switch (event.type) {
		...
		case "agent_end": {
			...
			return { type: "agent_end", messageCount: agentEndEvent.messages?.length ?? 0 };
		}
		case "turn_start":
...
async function resolveModel(requestedModel?: string): Promise<Model<any> | undefined> {
...
async function createSessionForRequest(workspaceDir: string, model?: Model<any>) {
...
async function getOrCreateSessionForRequest(request: OpenAiChatCompletionRequest, model?: Model<any>) {
	...
	if (!cacheKey) {
		return {
			...(await createSessionForRequest(workspaceDir, model)),
			cacheKey: undefined,
			isReusable: false,
			isNewSession: true,
		};
	...
	if (existing) {
		if ((existing.modelKey && modelKey && existing.modelKey !== modelKey) || existing.workspaceDir !== workspaceDir) {
			disposeCachedSession(cacheKey);
		} else {
			existing.lastUsedAt = Date.now();
			log("reusing cached session", { cacheKey, modelKey: existing.modelKey });
			return {
	...
	const sessionResult = await createSessionForRequest(workspaceDir, model);
	...
	log("cached new session", { cacheKey, modelKey });
	return {
		...sessionResult,
		cacheKey,
		isReusable: true,
