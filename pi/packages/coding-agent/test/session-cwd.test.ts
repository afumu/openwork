...
import { createInMemorySessionManagerForWorkspace } from "../src/runtime/session-workspace.js";
...
describe("session cwd handling", () => {
	...
	it("keeps the conversation workspace cwd when creating in-memory runtime sessions", () => {
		const conversationWorkspace = createTempDir("pi-session-cwd-conversation");
		cleanupPaths.push(conversationWorkspace);

		const sessionManager = createInMemorySessionManagerForWorkspace(conversationWorkspace);

		expect(sessionManager.getCwd()).toBe(conversationWorkspace);
	});
});
